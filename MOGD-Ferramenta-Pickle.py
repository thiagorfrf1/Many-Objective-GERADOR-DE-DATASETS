...
while tread == "":
    ...
    if ("1" in metricasList):
        objetivo = input("Escolha os valores que deseja alcançar para: Class imbalance C2")
        globalBalance = float(objetivo)
    if ("2" in metricasList):
        objetivo = input("Escolha os valores que deseja alcançar para: Linearity L2")
        globalLinear = float(objetivo)
    if ("3" in metricasList):
        objetivo = input("Escolha os valores que deseja alcançar para: Neighborhood N2")
        globalN2 = float(objetivo)
    if ("2" in metricasList):
        objetivo = input("Escolha os valores que deseja alcançar para: Network ClsCoef")
        globalClsCoef = float(objetivo)
    if ("2" in metricasList):
        objetivo = input("Escolha os valores que deseja alcançar para: Dimensionality T2")
        globalt2 = float(objetivo)
    if ("2" in metricasList):
        objetivo = input("Escolha os valores que deseja alcançar para: Feature-based F1")
